...
export function AuthProvider({ children }) {
  ...
  const [methods, setMethods] = useState({ entra: false, google: false, local: false, bootstrap: false });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    Promise.all([
      fetch('/auth/me', { credentials: 'include' })
        .then(r => r.ok ? r.json() : (r.status === 401 ? r.json().then(d => { if (d?.pendingMfa) setPendingMfa(true); return null; }).catch(() => null) : null)),
      fetch('/auth/methods', { credentials: 'include' })
  ...
  const loginEntra = () => { window.location.href = '/auth/login'; };
  const loginGoogle = () => { window.location.href = '/auth/google/login'; };
  ...
  return (
    <AuthContext.Provider value={{
      user, loading, pendingMfa, methods,
      loginEntra, loginGoogle, loginLocal, bootstrapLocal, submitMfa,
      logout, setUser, setDefaultProject,
    }}>
      {children}
