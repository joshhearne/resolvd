...
export default function MfaSetup() {
  ...
  async function regenerateCodes() {
    if (!confirm('This invalidates all previous recovery codes. Continue?')) return;
    setBusy(true);
    try {
    ...
    return (
      <div className="space-y-4 max-w-md">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
          <h2 className="text-sm font-semibold text-gray-900">Recovery codes</h2>
          <p className="text-xs text-gray-500">
            Lost your authenticator? Generate a fresh batch of one-time recovery codes.
          </p>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
          <h2 className="text-sm font-semibold text-gray-900">Disable MFA</h2>
          <p className="text-xs text-gray-500">
            Provide your current authenticator code, or your password (local accounts only).
          </p>
          <input type="text" placeholder="6-digit code" value={disableToken}
          ...
          <button onClick={disableMfa} disabled={busy}
...
{recoveryCodes.map(c => <span key={c}>{c}</span>)}
  ...
  if (step === 'enrolling' && qr) {
    return (
      <form onSubmit={confirmSetup} className="space-y-4 max-w-md">
        <h1 className="text-xl font-semibold text-gray-900">Set up MFA</h1>
        <p className="text-sm text-gray-600">
          Scan this QR code with Google Authenticator, Authy, 1Password, or any TOTP-compatible app.
        </p>
        <img src={qr.qrDataUrl} alt="MFA QR code" className="border border-gray-200 rounded" />
        <p className="text-xs text-gray-500">
          Or enter this secret manually: <code className="bg-gray-100 px-2 py-1 rounded">{qr.secret}</code>
        </p>
        <input
          autoFocus
          inputMode="numeric"
  ...
  return (
    <div className="space-y-4 max-w-md">
      <h1 className="text-xl font-semibold text-gray-900">Two-factor authentication</h1>
      <p className="text-sm text-gray-600">
        Add a second factor to your account using a TOTP authenticator app (Google Authenticator, Authy, 1Password, etc).
      </p>
