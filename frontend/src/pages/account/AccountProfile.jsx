...
export default function AccountProfile() {
  ...
  const onCropComplete = useCallback(
    (_area, areaPx) => setCroppedArea(areaPx),
    [],
  );
    ...
    try {
      ...
      const updated = {
        ...
        profilePictureUrl: data.user.profilePictureUrl
          ? `${data.user.profilePictureUrl}?t=${Date.now()}`
          : null,
      };
      setUser(updated);
    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="flex items-start gap-6">
          <Avatar user={user} size="xl" className="!bg-surface-2 !text-fg" />
            ...
            {isLocal ? (
              <>
                ...
                <div className="flex gap-2">
                  <label className="inline-flex items-center px-3 py-1.5 bg-brand hover:bg-brand-bright text-brand-fg text-sm rounded-md cursor-pointer">
                    Choose photo
                    <input
                      type="file"
                      accept="image/*"
                  ...
                  {user?.profilePictureUrl && (
                    <button
                      ...
                      className="px-3 py-1.5 border border-border-strong hover:bg-surface-2 text-sm rounded-md"
                    >
                      Remove
                    </button>
                  )}
              ...
              <p className="text-sm text-fg-muted">
                Your photo is synced from your{" "}
                <strong>
                  {user?.authProvider === "entra" ? "Microsoft" : "Google"}
                </strong>{" "}
                account on each sign-in.
              </p>
            )}
          </div>
        </div>
        ...
        {imageSrc && (
          <div className="mt-6 border border-border rounded-lg p-4 bg-surface-2">
            ...
            <div className="flex items-center gap-3 mt-3">
              <label className="text-xs text-fg-muted">Zoom</label>
              <input
                type="range"
                min={1}
                max={3}
      ...
      <section>
        ...
        <form onSubmit={saveDisplayName} className="flex gap-2 max-w-md">
          <input
            ...
            className="flex-1 border border-border-strong rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-brand/40"
            maxLength={120}
            required
          />
          <button
      ...
      <section>
        ...
        <dl className="grid grid-cols-1 sm:grid-cols-3 gap-y-2 gap-x-4 text-sm">
          ...
          <dd className="sm:col-span-2 text-fg">{user?.email || "—"}</dd>
          <dt className="text-fg-muted">Sign-in method</dt>
          <dd className="sm:col-span-2 text-fg capitalize">
            {user?.authProvider}
          </dd>
          <dt className="text-fg-muted">Role</dt>
          <dd className="sm:col-span-2 text-fg">{user?.role}</dd>
        </dl>
      </section>
    </div>
  );
