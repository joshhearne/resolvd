...
export default function AdminExport() {
  ...
  function buildQS() {
    ...
    const projectParam = allProjectsSelected
      ? ""
  ...
  function openExport() {
    const { statuses, projectParam, dateParam } = buildQS();
    window.open(
      `/print-export?statuses=${encodeURIComponent(statuses)}${projectParam}${dateParam}`,
      "_blank",
    );
  }
            ...
            {projects.map((p) => (
              ...
              >
                ...
                <span className="text-sm text-fg">
                  <span className="font-mono text-xs text-fg-dim mr-1">
                    {p.prefix}
                  </span>
                  {p.name}
                  {user?.defaultProjectId === p.id && (
      ...
      <div className="bg-surface rounded-lg border border-border p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2 text-xs">
            ...
            <span className="text-fg-dim">|</span>
            <button onClick={selectNone} className="text-brand hover:underline">
              None
            </button>
          </div>
        </div>
        ...
        <div className="pt-2 border-t border-border space-y-3">
          <p className="text-xs text-fg-muted">
            Export order: <strong>Blocked</strong> → <strong>Active</strong> →{" "}
            <strong>Closed</strong>. Within each group: priority then ticket
            number.
          </p>
          ...
          <div className="flex items-center gap-3">
            ...
            {counted && previewCount !== null && (
              <span className="text-sm text-fg-muted">
                <strong>{previewCount}</strong> ticket
