...
- **BYO-AI text rewrite — provider adapters + per-user config + per-surface composer button** — pluggable adapter pattern under `backend/services/aiProviders/` ships three out of the box: `openai` (compat with Azure / OpenRouter / vLLM / LM Studio), `anthropic` (Claude Messages API), `ollama` (self-hosted, no key). New providers drop in by adding a file + registering in `aiProviders/index.js`. Top-level `services/aiRewrite.js` builds tone × verbosity × ELI5 × surface-aware system prompts, dispatches to the right adapter, returns rewrite + token usage. Detects `{tag.path}` placeholders in input and adds a preservation directive so admin email templates keep their `{ticket.ref}`/`{vendor.name}` substitutions intact. Endpoints under `/api/ai`: `GET /providers`, `GET /config`, `PATCH /config`, `POST /api-key`, `POST /test`, `POST /rewrite`. **Per-user encrypted key** at rest via the standard envelope wrapper (`users.ai_api_key_enc`); never returned from any GET — only a `has_key` boolean. **Org-level kill switch** in Branding (`branding.ai_assist_enabled`, default ON) hides the AI surface everywhere when off. **ELI5 mode gated to Admin/Manager** server-side. Frontend: AccountPreferences → AI Assist card (provider dropdown, endpoint URL, model, API key with masked display + Replace/Remove, default tone+verbosity, Test connection button); AdminBranding → org toggle; new `<AiRewriteModal>` (preview-before-send: tone+verbosity+ELI5 selectors, side-by-side original/rewritten panes, provider+model+token usage badge, Cancel/Rewrite/Re-roll/Apply); `<AiRewriteButton>` for plain `<input>`/`<textarea>` surfaces; `MarkdownEditor` gains an `aiSurface` prop that adds a ✨ AI button to its toolbar. Wired into four surfaces: internal comment composer, vendor comment composer (auto-flips on Share-with-vendor toggle), ticket description (NewTicket + TicketDetail edit), and the admin email-template Subject + Body. MarkdownEditor default `rows` bumped 5 → 10 so the composer breathes.
