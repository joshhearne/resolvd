...
- **Runbook → ScreenConnect BackStage push.** Extends the runbook feature (v0.8.x — KB `kind='runbook'` + per-ticket `ticket_runbook_runs`). Today runbook steps are manual checklists with optional `@canned:` pills that prefill the comment composer. Next: when a runbook step references an SC BackStage script (`@sc:<script-id>` or similar), allow the agent to dispatch the script to the linked asset's endpoint via SC's REST API and capture stdout/stderr back into the runbook step. Gated on hosted multi-tenant SC reach + the deep-link launcher landing first.
- **Runbook → Action1 script push.** Same idea, but for Action1 deployments. Currently the Action1 integration is one-way (we pull endpoints / software / alerts). When A1 ships a usable script-deploy API path for partner integrations, runbook steps tagged `@action1:<deployment-id>` could fire a deployment against the ticket's linked asset and surface the result in the runbook step.
