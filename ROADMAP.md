...
- **Post-v0.7.0 patches** — small follow-ups landed on the v0.7.0 line, not yet tagged:
  - **Comment signature** — per-user optional sign-off appended to outgoing comments. New prefs `signature` (string, max 2000 chars), `append_signature` (toggle, default off), `append_signature_scope` (`vendor_only` default / `all`). Configured at Account Settings → Preferences → Comment signature with live preview. Composer in `TicketDetail` appends `\n\n${signature}` when the toggle is on and scope matches (vendor_only ⇒ only when Share with vendor is ticked). Empty signature short-circuits even if toggled on. Validated server-side in `routes/users.js` PATCH `/me/prefs`.
  - **Vendor reply attachments persisted + linked to the comment** — `tryAutoReply` (vendor reply on `[PREFIX-N]` threads) now accepts the inbound attachments array, persists each file, and links it to the new comment via `attachments.comment_id` — same shape as user-uploaded files via the composer. Dedup-append branch (`tryAutoCreate` exact-match) does the same: `appendCommentToTicket` returns its comment id, the persist loop passes it through. `persistAttachment` grew an optional `commentId` arg. UI side: `GET /api/tickets/:id/attachments` joins through comments → contacts → companies to surface `vendor_contact_id` + names; inline chips and the Attachments tab use `vendorPillStyle()` so vendor-attached files inherit the same color as the comment header (image thumbs get a colored border, file pills get the full vendor color, Attachments tab shows a "from VendorName" tag).
...
- **Per-location business hours** — `business_hours_policies` currently keys on `project_id`. Add `location_id` (nullable) so multi-site orgs can run different windows per branch under the same project. Resolution rank becomes location → project → org default. Needs either a `tickets.location_id` column or a derive-from-submitter path (`submitted_by` → `company_members.location_id`). Aspirational until a customer asks.
- **On-call rotation for notify_role / reassign_role escalations.** Today `notify_role` pages every active project member with that role and `reassign_role` picks the lowest-id agent — no concept of who's currently on call. Aspirational follow-up: schedule table (`oncall_shifts`) + a `notify_oncall` / `reassign_oncall` action kind. Punt until a customer asks for it.
- **Rules engine — IF/THEN behavior overrides.** Admin-configurable "when this matches, do that" overlay for SLA + assignment + notification behavior. Conditions composable via dropdown chips (field selector → operator → value), with regex and contains/equals/in operators, AND/OR groups, optional negation. Actions: skip SLA response clock, force a priority, swap assignee, suppress a notification, force a status transition, etc. First concrete use case: *"submitter == assigned agent AND not 'on behalf of' another user → do not require SLA response"*; the on-behalf-of distinction comes from `audit_log.action='submitted_on_behalf'` or an explicit flag on the ticket. Storage: `automation_rules` table (id, name, enabled, priority, trigger_event, conditions JSONB, actions JSONB, created_by). Evaluated at the same hooks where applyPolicyOnCreate / markResponded / notification fanout run. Big surface — break into discrete milestones (read-only rule preview → SLA-skip action only → assignment override → notification suppression → full IF/THEN editor) so something useful ships before the whole engine is done.
