...
- **Business-hours-aware escalation chains.** `escalation_chain_steps.delay_minutes` is added with wall-clock interval math in `tickEscalations()` (`backend/services/escalations.js:353`), and the fire moment itself isn't gated on the bh window. Result: a breach stamped at 4:55 PM CT with a 60-min step fires at 5:55 PM CT — outside hours — and a `bump_priority` action on that step bumps the ticket overnight. Fix shape: when the matching SLA policy has a `business_hours_id`, walk `triggered_at + delay_minutes` through `addBusinessMinutes()` instead of raw interval math, and short-circuit any fire moment that falls outside the window to the next window start. Single-source-of-truth: store as UTC, do the bh math on read. Mirrors the SLA due_at fix already in `applyPolicyOnCreate`.
